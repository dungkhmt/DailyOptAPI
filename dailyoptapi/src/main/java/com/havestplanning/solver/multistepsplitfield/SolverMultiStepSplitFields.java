package com.havestplanning.solver.multistepsplitfield;
...
import com.havestplanning.model.HavestPlanningCluster;
import com.havestplanning.model.HavestPlanningField;
...
public class SolverMultiStepSplitFields extends Solver {
	...
	public void extendDateSequence(Date fromDate, Date toDate) {
		...
		if (startDate.compareTo(date_sequence[0]) >= 0)
			startDate = date_sequence[0];
		Date lastDate = toDate;
		if (lastDate.compareTo(date_sequence[date_sequence.length - 1]) <= 0)
			lastDate = date_sequence[date_sequence.length - 1];
		System.out.println(name() + "::extendDateSequence, startDate = "
				+ DateTimeUtils.date2YYYYMMDD(startDate) + ", lastDate = "
				+ DateTimeUtils.date2YYYYMMDD(lastDate));
		...
		while (true) {
			...
			dateList.add(d);
			System.out.println(name() + "::extendDateSequence, dateList["
					+ (dateList.size() - 1) + "] = " + Utility.dateMonthYear(d)
					+ ", lastDate = " + Utility.dateMonthYear(lastDate));
			if (d.compareTo(lastDate) >= 0)
				break;
			curDate = d;
		}
	...
	public void search(int maxNbSteps, int timeLimit, int delta_left,
			int delta_right, String startDatePlanStr) {

		int nbHarvestDays = totalQuantity
				/ input.getMachineSetting().getMinLoad() + 1;
		...
		for (int i = 0; i < date_sequence.length; i++) {
			if (sdp.compareTo(date_sequence[i]) == 0) {
				sdp = date_sequence[i];
				break;
			}
		}
		...
		for (int i = 0; i < n; i++) {
			...
			if (L < maxDate[i])
				L = maxDate[i];
			if (min_range > maxDate[i] - minDate[i])
				min_range = maxDate[i] - minDate[i];
			if (max_range < maxDate[i] - minDate[i])
				max_range = maxDate[i] - minDate[i];
		}
		System.out.println(name() + "::search, n = " + n + ", m = " + m
				+ ", totalQuantity = " + totalQuantity + ", E = " + E
				+ ", L = " + L + ", D.sz = " + D.size() + ", min_rng = "
				+ min_range + ", max_rng = " + max_range + ", MAX = "
				+ (D.size() * 6000));
		// System.exit(-1);

		double[][] p = new double[n][m];
		/*
		 * for(int i = 0; i < n; i++){ Date date =
		 * DateTimeUtils.convertYYYYMMDD2Date(fields[i].getPlant_date()); int
		 * plantStart = mDate2Slot.get(date);
		 * 
		 * for(int d = 0; d < m; d++){
		 * 
		 * if(d < minDate[i] || d > maxDate[i]) p[i][d] = 0; else{ int period =
		 * d - plantStart; p[i][d] =
		 * input.getPlantStandard().evaluateQuality(fields[i].getCategory(),
		 * fields[i].getPlantType(), period); System.out.println(name() +
		 * "::search, SET p[" + i + "," + d + "] = " + p[i][d]); } } }
		 */
		...
		S.setInput(preload, qtt, minDate, maxDate, expected_dates, minLoad,
				maxLoad, p);
		S.stateModel();
		...
		for (int i = 0; i < load.length; i++)
			if (load[i] > 0) {
				if (initMinQuantityDay > load[i])
					initMinQuantityDay = load[i];
				if (initMaxQuantityDay < load[i])
					initMaxQuantityDay = load[i];
				if (load[i] < input.getMachineSetting().getMinLoad())
					numberOfDaysUnderLoad++;
				if (load[i] > input.getMachineSetting().getMaxLoad())
					numberOfDaysOverLoad++;
			}
		int[] b = new int[m];
		...
		while (nbSteps < maxNbSteps) {
			...
			LeveledHavestPlanSolution s = S.solve(preload, qtt, minDate,
					maxDate, expected_dates, minLoad, maxLoad, startDatePlan,
					timeLimit);
			solutions.add(s);
			// System.out.println(name() + "::search, solutions[" +
			// solutions.size() + "] = " + s.toString());
			...
			for (int i = 0; i < n; i++) {
				...
				// if(delta > 50){
				// System.out.println(name() + "::search, BUG, xd[" + i + "] = "
				// + xd[i] +
	...
	public InputAnalysisInfo analyze(HavestPlanningInput input) {
		...
		String latestPlantDate = DateTimeUtils
				.date2YYYYMMDD(dates[dates.length - 1]);
		int numberOfFields = input.getFields().length;
		int totalQuantity = 0;
		int numberOfDatesPlantRange = Utility.distance(earliestPlantDate,
				latestPlantDate);
		int maxQuantity = -1;
		...
		for (int i = 0; i < input.getFields().length; i++) {
			...
			if (minQuantity > f.getQuantity())
				minQuantity = f.getQuantity();
			if (maxQuantity < f.getQuantity())
				maxQuantity = f.getQuantity();
		}

		return new InputAnalysisInfo(earliestPlantDate, latestPlantDate,
				numberOfFields, totalQuantity, numberOfDatesPlantRange,
				maxQuantity, minQuantity);
	}
	public void logDateSequence(){
		for (int i = 0; i < date_sequence.length; i++) {
	...
	public HavestPlanningSolution solve(HavestPlanningInput input,
		...
		if (date_sequence.length > 2000) {
			String des = "field " + fields[0].getCode() + " with plant_date = "
					+ fields[0].getPlant_date() + ", field "
					+ fields[fields.length - 1].getCode()
					+ " with plant_date = "
					+ fields[fields.length - 1].getPlant_date()
					+ ", maxPeriod = "
					+ input.getPlantStandard().getMaxPeriod();
		...
		for (int i = 0; i < day_planned.length; i++)
			day_planned[i] = 0;
			...
			for (int i = 0; i < date_sequence.length; i++) {
				...
				if (F.size() > 0) {
					...
					for (int j = 0; j < F.size(); j++) {
						...
						if (days_late < 0) {
							if (maxDaysEarly < (-days_late))
								maxDaysEarly = -days_late;
						} else {
							if (maxDaysLate < days_late)
					...
					// HavestPlanningCluster Ck = new
					// HavestPlanningCluster(date,
		...
		numberOfDaysPlanned = 0;
		for (int i = 0; i < day_planned.length; i++)
			numberOfDaysPlanned += day_planned[i];

		for (int i = 0; i < fields.length; i++)
			totalQuantity += fields[i].getQuantity();

		for (int i = 0; i < cluster.size(); i++)
			if (cluster.get(i).getQuantity() > 0) {
		...
		for (int i = 0; i < a_cluster.length; i++) {
			...
			if (c.getQuantity() < input.getMachineSetting().getMinLoad())
				numberOfDaysUnderLoad++;
			if (c.getQuantity() > input.getMachineSetting().getMaxLoad())
				numberOfDaysOverLoad++;
		}

		// HavestPlanningSolution solution = new
		// HavestPlanningSolution(a_cluster, quality);
		HavestPlanningSolution solution = new HavestPlanningSolution(quality,
				"success", numberOfFieldsInPlan, numberOfDatesInPlan,
				numberOfDatesInPlantStandard, initMinQuantityDay,
				initMaxQuantityDay, computedMinQuantityDay,
				computedMaxQuantityDay, 0, quantityNotPlanned, quantityPlanned,
				totalQuantity, numberLevels, numberOfDaysHarvestExact,
				numberOfDaysPlanned, numberOfFieldsCompleted, maxDaysLate,
				maxDaysEarly, numberOfDaysOverLoad, numberOfDaysUnderLoad);
