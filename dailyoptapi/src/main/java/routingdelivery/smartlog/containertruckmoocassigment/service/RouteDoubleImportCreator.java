...
public class RouteDoubleImportCreator {
	...
	public boolean checkCapacityForDoubleImport(Truck truck, Mooc mooc,
		...
		if(!ir1.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY20)
				|| !ir2.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY20)) return false;
		if(mooc.getCategory().equals(MoocCategoryEnum.CATEGORY20)) return false;
		
		return true;
	}
	public TruckRoute createDoubleImportRequest(Truck truck, Mooc mooc,
			ImportContainerRequest ir1, ImportContainerRequest ir2) {
		// truck-mooc -> port(ir1) -> port(ir2) -> warehouse(ir1/ir2) ->
		// warehouse(ir2/ir1)
		//System.out.println(name() + "::createDoubleImportRequest, ir1.portCode = " + ir1.getPortCode());
		
		if(!checkCapacityForDoubleImport(truck, mooc, ir1, ir2)) return null;
		...
		Container container1 = solver.mCode2Container.get(ir1.getContainerCode());
		Container container2 = solver.mCode2Container.get(ir2.getContainerCode());
		...
		if (ir1First) {
			...
			if (arrivalTime > (int) DateTimeUtils.dateTime2Int(ir1
				...
				return null;
			serviceTime = Utils.MAX(arrivalTime, (int) DateTimeUtils.dateTime2Int(ir1
			...
			if (arrivalTime > (int) DateTimeUtils.dateTime2Int(ir2
					.getLateDateTimePickupAtPort()))
				return null;
			serviceTime = Utils.MAX(arrivalTime, (int) DateTimeUtils.dateTime2Int(ir2
					.getEarlyDateTimePickupAtPort()));
		...
		ir1First = true;
		if (solver.getDistance(e3.getPort().getLocationCode(), wh1.getLocationCode()) > solver.getDistance(
				e3.getPort().getLocationCode(), wh2.getLocationCode())) {
			ir1First = false;
		}
		...
		if (ir1First) {
			...
			//	System.out.println(name() + "::createDoubleImportRequest, warehouse e5 = NULL");
			//}else{
			//	System.out.println(name() + "::createDoubleImportRequest, warehouse e5 = " + e5.getWarehouse().getLocationCode());
			//}
			arrivalTime = departureTime
					+ solver.getTravelTime(e5.getWarehouse().getLocationCode(), e6
							.getWarehouse().getLocationCode());
			if (arrivalTime > (int) DateTimeUtils.dateTime2Int(ir2
					.getLateDateTimeUnloadAtWarehouse()))
				return null;
		...
		DepotContainer depotContainer1 = solver.findDepotForReleaseContainer(container1);
		DepotContainer depotContainer2 = solver.findDepotForReleaseContainer(container2);

		if(depotContainer1 == null){
			depotContainer1 = solver.mCode2DepotContainer.get(ir1.getDepotContainerCode());
		}
		if(depotContainer2 == null){
			depotContainer2 = solver.mCode2DepotContainer.get(ir2.getDepotContainerCode());
		}
		
		
		//if(depotContainer1 == null){
		//	System.out.println(name() + "::createDoubleImportRequest depotContainer1 = NULL");
		//}else{
		//	System.out.println(name() + "::createDoubleImportRequest depotContainer1 = " + depotContainer1.getLocationCode());
		//}
		ir1First = true;
		if (solver.getDistance(e7.getWarehouse().getLocationCode(),
				depotContainer1.getLocationCode()) > solver.getDistance(e7
				.getWarehouse().getLocationCode(),
				depotContainer2.getLocationCode())) {
			ir1First = false;
		}
		...
		if (ir1First) {
			...
			arrivalTime = departureTime + solver.getTravelTime(e7.getWarehouse().getLocationCode(), e8.getDepotContainer().getLocationCode());
			serviceTime = arrivalTime;
			...
			arrivalTime = departureTime + solver.getTravelTime(e8.getDepotContainer().getLocationCode(),e9.getDepotContainer().getLocationCode());
			serviceTime = arrivalTime;
