...
public class RouteTangboWarehouseExport {
	...
	public boolean checkTangboWarehouseExport(Truck truck, Mooc mooc,
		...
		if(mooc.getCategory().equals(MoocCategoryEnum.CATEGORY20)){
			if(!container.getCategoryCode().equals(ContainerCategoryEnum.CATEGORY20)) ok = false;
		}else if(mooc.getCategory().equals(MoocCategoryEnum.CATEGORY40)){
			if(container.getCategoryCode().equals(ContainerCategoryEnum.CATEGORY45)) ok = false;
		} 
		if(!ok) return ok;
		
		if(container.getCategoryCode().equals(ContainerCategoryEnum.CATEGORY20)){
			if(!wr.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY20)) ok = false;
			...
			if(wr.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY45)) ok = false;
			if(er.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY45)) ok = false;
		} 
		
		return ok;
	}
	public TruckRoute createTangboWarehouseExport(Truck truck, Mooc mooc,
			Container container, WarehouseContainerTransportRequest wr,
			ExportContainerRequest er) {
		...
		if(!checkTangboWarehouseExport(truck, mooc, container, wr, er)) return null;
		if(solver.mContainer2LastDepot.get(container) == null){
			// this is imported container, does not has depot
			solver.logln(name() + "::createTangboWarehouseExport, imported container null");
			return null;
		}
		...
		e3.setWarehouse(solver.mCode2Warehouse.get(wr.getFromWarehouseCode()));
		e3.setWarehouseRequest(wr);
		arrivalTime = departureTime
				+ solver.getTravelTime(e2.getDepotContainer().getLocationCode(), e3
						.getWarehouse().getLocationCode());
		if (arrivalTime > DateTimeUtils.dateTime2Int(wr.getLateDateTimeLoad())){
			solver.logln(name() + "::createTangboWarehouseExport, violation time arrival = " + 
		DateTimeUtils.unixTimeStamp2DateTime(arrivalTime) + " > wr.getLateDateTimeLoad() = " + wr.getLateDateTimeLoad());
			return null;
		}
		...
		e4.setWarehouse(solver.mCode2Warehouse.get(wr.getFromWarehouseCode()));
		...
		if (arrivalTime > DateTimeUtils
			...
			solver.logln(name() + "::createTangboWarehouseExport, violation time arrival = " + 
					DateTimeUtils.unixTimeStamp2DateTime(arrivalTime) + " > wr.getLateDateTimeUnload() = " + wr.getLateDateTimeUnload());
			return null;
		}
		serviceTime = Utils.MAX(arrivalTime,
		...
		if (arrivalTime > DateTimeUtils.dateTime2Int(er
			...
			solver.logln(name() + "::createTangboWarehouseExport, violation time arrival = " + 
					DateTimeUtils.unixTimeStamp2DateTime(arrivalTime) + " > er.getLateDateTimeLoadAtWarehouse() = " + er
					.getLateDateTimeLoadAtWarehouse());
			return null;
		}
		...
		if (arrivalTime > DateTimeUtils.dateTime2Int(er
			...
			solver.logln(name() + "::createTangboWarehouseExport, violation time arrival = " + 
					DateTimeUtils.unixTimeStamp2DateTime(arrivalTime) + " > er.getLateDateTimeUnloadAtPort() = " + er
					.getLateDateTimeUnloadAtPort());
			return null;
		}
		serviceTime = Utils.MAX(arrivalTime, (int) DateTimeUtils.dateTime2Int(er
		...
		arrivalTime = departureTime
				+ solver.getTravelTime(e10.getDepotMooc().getLocationCode(), e11
