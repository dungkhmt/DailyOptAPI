...
public class RouteKeplechCreator {
	...
	public boolean checkKeplech(Truck truck, Mooc mooc,
			Container container, ExportContainerRequest er,
			ImportContainerRequest ir){
		boolean ok = true;
		if(container.getCategoryCode().equals(ContainerCategoryEnum.CATEGORY20)){
			if(!er.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY20))ok = false;
		} else if(container.getCategoryCode().equals(ContainerCategoryEnum.CATEGORY40)){
			if(er.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY45)) ok = false;
			
		}
		if(!ok) return ok;
		if(mooc.getCategory().equals(MoocCategoryEnum.CATEGORY20)){
			if(!ir.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY20)) ok = false;
		}else if(mooc.getCategory().equals(MoocCategoryEnum.CATEGORY40)){
			if(ir.getContainerCategory().equals(ContainerCategoryEnum.CATEGORY45)) ok = false;
		} 
		
		return ok;
	}
	public TruckRoute createKeplech(Truck truck, Mooc mooc,
			Container container, ExportContainerRequest er,
			ImportContainerRequest ir) {
		// truck-mooc-container -> warehouse(er) -> port(ir) -> port(er) ->
		// warehouse(ir)
		if(!checkKeplech(truck, mooc, container, er, ir)) return null;
		...
		int departureTime = solver.mTruck2LastTime.get(truck);
		...
		int arrivalTime = departureTime
				+ solver.getTravelTime(e0.getDepotTruck().getLocationCode(), e1
		...
		serviceTime = Utils.MAX(arrivalTime, solver.mContainer2LastTime.get(container));
		...
		arrivalTime = departureTime
				+ solver.getTravelTime(e2
						.getDepotContainer().getLocationCode(),e3.getWarehouse().getLocationCode());
		if(arrivalTime > DateTimeUtils.dateTime2Int(er.getLateDateTimeLoadAtWarehouse())) return null;
		serviceTime = Utils.MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(er.getEarlyDateTimeLoadAtWarehouse()));
		duration = 0;
		departureTime = serviceTime + duration;
		...
		arrivalTime = departureTime + solver.getTravelTime(e4.getWarehouse().getLocationCode(), e5.getPort().getLocationCode());
		if(arrivalTime > DateTimeUtils.dateTime2Int(er.getLateDateTimeUnloadAtPort())) return null;
		
		serviceTime = Utils.MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(er.getEarlyDateTimeUnloadAtPort()));
		...
		Container import_container = solver.mCode2Container.get(ir.getContainerCode());
		e6.setContainer(import_container);
		e6.setImportRequest(ir);
		arrivalTime = departureTime + solver.getTravelTime(e5.getPort().getLocationCode(), e6.getPort().getLocationCode());
		if(arrivalTime > DateTimeUtils.dateTime2Int(ir.getLateDateTimePickupAtPort())) return null;
		
		serviceTime = Utils.MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(ir.getEarlyDateTimePickupAtPort()));
		...
		arrivalTime = departureTime + solver.getTravelTime(e6.getPort().getLocationCode(),e7.getWarehouse().getLocationCode());
		if(arrivalTime > DateTimeUtils.dateTime2Int(ir.getLateDateTimeUnloadAtWarehouse())) return null;
		
		serviceTime = Utils.MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(ir.getEarlyDateTimeUnloadAtWarehouse()));
		...
		DepotContainer depotContainer = solver.findDepotForReleaseContainer(import_container);
		if(depotContainer == null){
			depotContainer = solver.mCode2DepotContainer.get(ir.getDepotContainerCode());
		}
		...
		arrivalTime = departureTime + solver.getTravelTime(e8.getWarehouse().getLocationCode(), e9.getDepotContainer().getLocationCode());
		serviceTime = arrivalTime;
		...
		arrivalTime = departureTime + solver.getTravelTime( e9.getDepotContainer().getLocationCode(),e10.getDepotMooc().getLocationCode());
		serviceTime = arrivalTime;
		...
		arrivalTime = departureTime + solver.getTravelTime(e10.getDepotMooc().getLocationCode(),e11.getDepotTruck().getLocationCode());
		serviceTime = arrivalTime;
		duration = 0;
		departureTime = serviceTime + duration;
