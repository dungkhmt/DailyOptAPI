...
public class ContainerTruckMoocSolver {
	...
	public ComboContainerMoocTruck createLastAvailableCombo(Truck truck,
		...
		ComboContainerMoocTruck ret = new ComboContainerMoocTruck(this,truck, mooc,
	...
	public ComboContainerMoocTruck createLastAvailableCombo(Truck truck, String locationTruck, int departureTime,
			double extraDistance, Mooc mooc, Container container) {
			...
			for (int i = tr.getNodes().length - 1; i >= 0; i--) {
				...
				if (e.getAction().equals(
					...
					if (e.getMooc() == mooc) {
						...
						ComboContainerMoocTruck ret = new ComboContainerMoocTruck(this,
				...
				}else if(e.getAction().equals(ActionEnum.RELEASE_CONTAINER_AT_DEPOT)){
					if(e.getMooc() == mooc){
						departureTime = mPoint2DepartureTime.get(e);
						locationCode = e.getLocationCode();
						String locationContainer = mContainer2LastDepot.get(
					...
					return createLastAvailableCombo(truck, locationCode, departureTime, extraDistance, mooc, container);
				}
			}
	...
	*/
	...
	public TruckRoute createDirectRouteForImportRequest(
		...
		SequenceSolution ss = SS.solve(lastElement.getLocationCode(), departureTime, req, null);
		int[]seq = ss.seq;
		...
		for(int i = 0; i < L.size(); i++) e[i] = L.get(i);
	...
	public TruckRouteInfo4Request createRouteForEmptyContainerToDepotRequest(
			EmptyContainerToDepotRequest req, Truck truck, Mooc mooc, Container container
			){
		...
		arrivalTime = departureTime + travelTime;
		if(arrivalTime >DateTimeUtils.dateTime2Int(req.getLateArrivalDateTime())) return null;
		startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(req.getEarlyArrivalDateTime()));
		...
		L.add(e[3]);
		e[3].deriveFrom(e[2]);
		DepotContainer depotContainer = findDepotContainer4Deposit(req, e[2], container);
	...
	public TruckRouteInfo4Request createRouteForTransportContainerRequest(
		TransportContainerRequest req, Truck truck, Mooc mooc, Container container){
	
		String header = name() + "::createRouteForEmptyContainerFromDepotRequest";
		ComboContainerMoocTruck combo = findLastAvailable(truck, mooc);
		...
		for(int i = 0; i < req.getLocations().length; i++){
			...
			if(arrivalTime > DateTimeUtils.dateTime2Int(tci.getLateArrivalTime())) return null;
			startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(tci.getEarlyArrivalTime()));
			duration = req.getAttachContainerDuration();
			departureTime = startServiceTime + duration;
	...
	public TruckRouteInfo4Request createRouteForEmptyContainerFromDepotRequest(
			EmptyContainerFromDepotRequest req, Truck truck, Mooc mooc, Container container
			){
		String header = name() + "::createRouteForEmptyContainerFromDepotRequest";
		...
		startServiceTime = MAX(arrivalTime,
				(int) DateTimeUtils.dateTime2Int(req
						.getEarlyArrivalDateTime()));
		distance = combo.extraDistance + getDistance(lastElement, e[3]);
		
		int finishedServiceTime = startServiceTime + req.getDetachLoadedMoocContainerDuration();// req.getLoadDuration();
		//duration = 0;
	...
	public TruckRouteInfo4Request createRouteForImportRequest(
		...
		arrivalTime = departureTime + travelTime;
		if(arrivalTime > DateTimeUtils.dateTime2Int(req.getLateDateTimePickupAtPort())) return null;
		startServiceTime = arrivalTime;
		duration = req.getLoadDuration();
		departureTime = startServiceTime + duration;
		mPoint2ArrivalTime.put(e2, arrivalTime);
		...
		for(int i = 0; i < seq.length; i++){
			...
			startServiceTime = MAX(arrivalTime,
					(int) DateTimeUtils.dateTime2Int(dwi
							.getEarlyDateTimeUnloadAtWarehouse()));
			int finishedServiceTime = startServiceTime + dwi.getDetachLoadedMoocContainerDuration();//req.getUnloadDuration();
			duration = 0;
			departureTime = finishedServiceTime;//startServiceTime + duration;
			mPoint2ArrivalTime.put(re[idx], arrivalTime);
			mPoint2DepartureTime.put(re[idx], departureTime);
			...
			travelTime = getTravelTime(lastElement, re[idx]);
			arrivalTime = departureTime + travelTime;
			if(arrivalTime > DateTimeUtils.dateTime2Int(dwi.getLatePickupEmptyContainerAtWarehouse())) return null;
			
			startServiceTime = MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(dwi.getEarlyPickupEmptyContainerAtWarehouse()));
			duration = 0;
			departureTime = startServiceTime + dwi.getAttachEmptyMoocContainerDuration();//duration;
			mPoint2ArrivalTime.put(re[idx], arrivalTime);
			mPoint2DepartureTime.put(re[idx], departureTime);
			lastElement = re[idx];
		}
		...
		DepotContainer depotContainer = findDepotContainer4Deposit(req, lastElement, container);
	...
	public TruckRoute createDirectRouteForExportRequest(
		...
		for(int i = 0; i < seq.length; i++){
			...
			arrivalTime = departureTime + travelTime;
			startServiceTime = MAX(arrivalTime,
					(int) DateTimeUtils.dateTime2Int(pwh.getEarlyDateTimeLoadAtWarehouse()));
	...
	public DepotMooc findDepotMooc4Deposit(EmptyContainerFromDepotRequest r, RouteElement fromElement, Mooc mooc){
		//return mCode2DepotMooc.get(mooc.getDepotMoocCode());
		double minDis = Integer.MAX_VALUE;
		DepotMooc sel_depot = null;
		for(String lc: mooc.getReturnDepotCodes()){
	...
	public DepotMooc findDepotMooc4Deposit(EmptyContainerToDepotRequest r, RouteElement fromElement, Mooc mooc){
		//return mCode2DepotMooc.get(mooc.getDepotMoocCode());
		double minDis = Integer.MAX_VALUE;
		DepotMooc sel_depot = null;
		for(String lc: mooc.getReturnDepotCodes()){
	...
	public DepotMooc findDepotMooc4Deposit(TransportContainerRequest r, RouteElement fromElement, Mooc mooc){
		//return mCode2DepotMooc.get(mooc.getDepotMoocCode());
		double minDis = Integer.MAX_VALUE;
		DepotMooc sel_depot = null;
		for(String lc: mooc.getReturnDepotCodes()){
	...
	public DepotContainer findDepotContainer4Deposit(EmptyContainerToDepotRequest r, RouteElement fromElement, Container container){
		double minDis = Integer.MAX_VALUE;
		DepotContainer sel_depot = null;
	...
	public DepotTruck findDepotTruck4Deposit(EmptyContainerFromDepotRequest r, RouteElement fromElement, Truck truck){
		//return mCode2DepotTruck.get(truck.getDepotTruckCode());
		double minDis = Integer.MAX_VALUE;
		DepotTruck sel_depot = null;
		for(String c: truck.getReturnDepotCodes()){
	...
	public DepotTruck findDepotTruck4Deposit(EmptyContainerToDepotRequest r, RouteElement fromElement, Truck truck){
		//return mCode2DepotTruck.get(truck.getDepotTruckCode());
		double minDis = Integer.MAX_VALUE;
		DepotTruck sel_depot = null;
		for(String c: truck.getReturnDepotCodes()){
	...
	public DepotTruck findDepotTruck4Deposit(TransportContainerRequest r, RouteElement fromElement, Truck truck){
		//return mCode2DepotTruck.get(truck.getDepotTruckCode());
		double minDis = Integer.MAX_VALUE;
		DepotTruck sel_depot = null;
		for(String c: truck.getReturnDepotCodes()){
	...
	public TruckRouteInfo4Request createRouteForExportRequest(
		...
		SequenceSolver SS = new SequenceSolver(this);
		SequenceSolution ss = SS.solve(lastElement.getLocationCode(), departureTime, req, port.getLocationCode());
		int[] seq = ss.seq;
		...
		for(int i = 0; i < seq.length; i++){
			...
			arrivalTime = departureTime + travelTime;
			// check time
			if(arrivalTime > DateTimeUtils.dateTime2Int(req.getLateDateTimeLoadAtWarehouse())) return null;
			...
			int finishedServiceTime = startServiceTime + pwi.getDetachEmptyMoocContainerDuration();// req.getLoadDuration();
			//duration = 0;
			departureTime = finishedServiceTime;//startServiceTime + duration;
			mPoint2ArrivalTime.put(re[idx], arrivalTime);
			mPoint2DepartureTime.put(re[idx], departureTime);
			...
			travelTime = getTravelTime(lastElement, re[idx]);
			arrivalTime = departureTime + travelTime;
			if(arrivalTime > DateTimeUtils.dateTime2Int(pwi.getLateDateTimePickupLoadedContainerAtWarehouse())) 
				return null;
			startServiceTime = MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(pwi.getEarlyDateTimePickupLoadedContainerAtWarehouse()));
			finishedServiceTime = startServiceTime + pwi.getAttachLoadedMoocContainerDuration();
	...
	public TruckRouteInfo4Request createRouteForWarehouseWarehouseRequest(
		...
		//System.out.println(name() + "::createRouteForWarehouseWarehouseRequest, combo = " + combo.toString());
		double distance = -1;
		...
		distance = combo.extraDistance + getDistance(lastElement, e[3]);
		arrivalTime = departureTime + travelTime;
		if(arrivalTime > DateTimeUtils.dateTime2Int(req.getLateDateTimeLoad())) return null;
		
		startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(req.getEarlyDateTimeLoad()));
		
		int finishedServiceTime = startServiceTime + req.getDetachEmptyMoocContainerDurationFromWarehouse();// req.getLoadDuration();
		duration = 0;
		departureTime = finishedServiceTime;//startServiceTime + duration;
		mPoint2ArrivalTime.put(e[3], arrivalTime);
		mPoint2DepartureTime.put(e[3], departureTime);
		...
		travelTime = getTravelTime(e[3], e[4]);
		arrivalTime = departureTime + travelTime;
		if(arrivalTime > DateTimeUtils.dateTime2Int(req.getLateDateTimePickupLoadedContainerFromWarehouse()))
			return null;
		startServiceTime = MAX(arrivalTime, (int)DateTimeUtils.dateTime2Int(req.getEarlyDateTimePickupLoadedContainerFromWarehouse()));//finishedServiceTime;
		duration = 0;
		departureTime = startServiceTime + req.getAttachLoadedMoocContainerDurationFromWarehouse();//duration;
		mPoint2ArrivalTime.put(e[4], arrivalTime);
		mPoint2DepartureTime.put(e[4], departureTime);
		...
		travelTime = getTravelTime(e[4], e[5]);
		arrivalTime = departureTime + travelTime;
		if(arrivalTime > DateTimeUtils.dateTime2Int(req.getLateDateTimeUnload())) return null;
		
		startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(req.getEarlyDateTimeUnload()));
		finishedServiceTime = startServiceTime + req.getDetachLoadedMoocContainerDurationToWarehouse();//req.getUnloadDuration();
		duration = 0;
		departureTime = finishedServiceTime;//startServiceTime + duration;
		mPoint2ArrivalTime.put(e[5], arrivalTime);
		mPoint2DepartureTime.put(e[5], departureTime);
		...
		travelTime = getTravelTime(e[5], e[6]);
		arrivalTime = departureTime + travelTime;
		if(arrivalTime > DateTimeUtils.dateTime2Int(req.getEarlyDateTimePickupEmptyContainerToWarehouse())) 
			return null;
		startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(req.getEarlyDateTimePickupEmptyContainerToWarehouse()));//finishedServiceTime;
		duration = 0;
		departureTime = startServiceTime + req.getAttachEmptyMoocContainerDurationToWarehouse();//duration;
		mPoint2ArrivalTime.put(e[6], arrivalTime);
		mPoint2DepartureTime.put(e[6], departureTime);
	...
	/*
	public TruckRouteInfo4Request createRouteForWarehouseWarehouseRequestSegments123(
			WarehouseContainerTransportRequest req, Truck truck, Mooc mooc,
			Container container) {
	...
	public TruckRouteInfo4Request createRouteForWarehouseWarehouseRequestSegment1(
			WarehouseContainerTransportRequest req, Truck truck, Mooc mooc,
			Container container) {
		...
		e[8].setAction(ActionEnum.RELEASE_MOOC_AT_DEPOT);
		e[8].setMooc(mooc);
		travelTime = getTravelTime(e[3], e[8]);
		arrivalTime = departureTime + travelTime;
		startServiceTime = arrivalTime;
		duration = mCode2DepotMooc.get(mooc.getDepotMoocCode())
				.getDeliveryMoocDuration();
	...
	public TruckRouteInfo4Request createRouteForWarehouseWarehouseRequestSegment2(
			WarehouseContainerTransportRequest req, Truck truck, Mooc mooc,
			Container container) {
		...
		startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(req.getEarlyDateTimeUnload()));
		int finishedServiceTime = startServiceTime + req.getDetachLoadedMoocContainerDurationToWarehouse();//req.getUnloadDuration();
		duration = 0;
		departureTime = finishedServiceTime;//startServiceTime + duration;
		mPoint2ArrivalTime.put(e[5], arrivalTime);
		mPoint2DepartureTime.put(e[5], departureTime);
		...
		e[8].setAction(ActionEnum.RELEASE_MOOC_AT_DEPOT);
		e[8].setMooc(mooc);
		travelTime = getTravelTime(e[5], e[8]);
		arrivalTime = departureTime + travelTime;
		startServiceTime = arrivalTime;
		duration = mCode2DepotMooc.get(mooc.getDepotMoocCode())
				.getDeliveryMoocDuration();
	...
	public TruckRouteInfo4Request createRouteForWarehouseWarehouseRequestSegment3(
			WarehouseContainerTransportRequest req, Truck truck, Mooc mooc,
			Container container) {
		...
		L.add(e[6]);
		e[6].deriveFrom(lastElement);
		e[6].setWarehouse(deliveryWarehouse);
		e[6].setAction(ActionEnum.LINK_EMPTY_CONTAINER_AT_WAREHOUSE);
		e[6].setWarehouseRequest(null);
		travelTime = getTravelTime(lastElement, e[6]);
		arrivalTime = departureTime + travelTime;
		if(arrivalTime > DateTimeUtils.dateTime2Int(req.getEarlyDateTimePickupEmptyContainerToWarehouse())) 
			return null;
		startServiceTime = MAX(arrivalTime,(int)DateTimeUtils.dateTime2Int(req.getEarlyDateTimePickupEmptyContainerToWarehouse()));//finishedServiceTime;
	...
	*/
	...
	public void propagateDistance(TruckRoute tr) {
		...
		//System.out.println(name() + "::propagateDistance, nodes.sz = "+ e.length);
		//for(int i = 0; i < e.length; i++)
		//	System.out.println(name() + "::propagateDistance, e[" + i + "] = " + e[i].getLocationCode());
		
		e[0].setDistance(0);
		for (int i = 1; i < e.length; i++) {
			//System.out.println(name() + "::propagateDistance, e[" + (i-1) + "] = " + e[i-1].getLocationCode() + 
	...
	public EmptyContainerFromDepotRequest[] getSortedEmptyContainerFromDepotRequests(){
		EmptyContainerFromDepotRequest[] L = new EmptyContainerFromDepotRequest[input.getEmptyContainerFromDepotRequests().length];
		...
		for(int i = 0; i < L.length-1; i++){
			for(int j = i+1; j<L.length; j++){
				long ti = DateTimeUtils.dateTime2Int(L[i].getLateArrivalDateTime());
				long tj = DateTimeUtils.dateTime2Int(L[j].getLateArrivalDateTime());
				if(ti > tj){
	...
	public EmptyContainerToDepotRequest[] getSortedEmptyContainerToDepotRequests(){
		EmptyContainerToDepotRequest[] L = new EmptyContainerToDepotRequest[input.getEmptyContainerToDepotRequests().length];
	...
	public TransportContainerRequest[] getSortedTransportContainerRequests(){
		TransportContainerRequest[] L = new TransportContainerRequest[input.getTransportContainerRequests().length];
		for(int i = 0; i < L.length; i++) L[i] = input.getTransportContainerRequests()[i];
		/*
		for(int i = 0; i < L.length-1; i++){
			for(int j = i+1; j<L.length; j++){
				long ti = DateTimeUtils.dateTime2Int(L[i].getLateArrivalDateTimeOrigin());
				long tj = DateTimeUtils.dateTime2Int(L[j].getLateArrivalDateTimeOrigin());
				if(ti > tj){
					TransportContainerRequest tmp = L[i];
					L[i] = L[j];
					L[j] = tmp;
				}
			}
