...
public class TruckItinerary {
	...
	private ArrayList<Integer> lastUsedIndex;// lastUsedIndex.get(i) is the index of the last RouteElement used
											// in the TruckRoute routes.get(i);
											// by default, lastUsedIndex.get(i) = routes.get(i).size() - 1
	                                        // if routes.get(i) has its successor routes.get(i+1), then
											// lastUsedIndex.get(i) may be different from routes.get(i).size() -1
											// as the truck does not return to the depot, instead it goes to the 
											// successor routes.get(i+1) from lastUsedIndex.get(i)
